...
static void getRegisterPressures(
  ...
  unsigned AddressableArchVGPR = DAG->MF.getSubtarget<GCNSubtarget>().getAddressableNumArchVGPRs();
  ...
  Pressure[AMDGPU::RegisterPressureSets::AGPR_32] = NewPressure.getAGPRNum(AddressableArchVGPR);
}
...
void GCNSchedStrategy::pickNodeFromQueue(SchedBoundary &Zone,
  ...
  if (DAG->isTrackingPressure()) {
    if (!GCNTrackers) {
      ...
      VGPRPressure = T->getPressure().getArchVGPRNum(DAG->MF.getSubtarget<GCNSubtarget>().getAddressableNumArchVGPRs());
    }
  }
...
void GCNSchedStage::checkScheduling() {
  ...
  if (PressureAfter.getSGPRNum() <= S.SGPRCriticalLimit &&
      PressureAfter.getVGPRNum(ST.hasGFX90AInsts(), AddressableArchVGPR) <= S.VGPRCriticalLimit) {
