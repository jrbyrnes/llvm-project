...
static cl::opt<bool> RewriteMFMAToAGPR(
    "amdgpu-rewrite-mfma-agpr", cl::Hidden,
    cl::desc("Whether or not to rewrite MFMA to AGPR form during scheduling."),
    cl::init(false));
...
void GCNScheduleDAGMILive::finalizeSchedule() {
  ...
  RegionsWithMFMAInstrs.resize(Regions.size());
  RescheduleRegions.resize(Regions.size());
  RegionsWithHighRP.reset();
  RegionsWithExcessRP.reset();
  RegionsWithMinOcc.reset();
  RegionsWithIGLPInstrs.reset();
  RegionsWithMFMAInstrs.reset();
...
bool GCNSchedStage::initGCNRegion() {
  ...
  if (StageID == GCNSchedStageID::OccInitialSchedule ||
    ...
    for (auto &I : DAG) {
      ...
      if (SII->isMAI(I))
        DAG.RegionsWithMFMAInstrs[RegionIdx] = true;
    }
  } else {
    for (auto &I : DAG)
      Unsched.push_back(&I);
...
bool PreRARematStage::createRematPlan(bool Aggressive) {
  ...
  for (unsigned I = 0, E = DAG.Regions.size(); I != E; ++I) {
    ...
    unsigned NumVGPRs = RP.getVGPRNum(ST.hasGFX90AInsts(), ST.getAddressableNumArchVGPRs());
...
bool RewriteScheduleStage::rewrite(
  ...
  DenseMap<MachineBasicBlock *,
           DenseMap<Register, SmallPtrSet<MachineOperand *, 8>>>
      ReachingUseTracker;
