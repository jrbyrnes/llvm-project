...
static cl::opt<bool> GeneralizedExpInterleave(
    "amdgpu-igrouplp-generalized-exp-interleave", cl::init(false), cl::Hidden,
    cl::desc(""));
...
private:
  ...
  public:
    ...
    LessThanNSuccs(unsigned Size, const SIInstrInfo *TII, unsigned SGID,
                   bool HasIntermediary = false,
                   bool NeedsCache = false)
        : InstructionRule(TII, SGID, NeedsCache), Size(Size), HasIntermediary(HasIntermediary) {}
  };
...
void MFMAExpInterleaveOpt::applyIGLPStrategy(
  ...
  if (!IsPostRA) {
    ...
    for (auto Succ : (*TempExp)->Succs) {
      if (isCvt(Succ.getSUnit()->getInstr()->getOpcode())) {
        FoundCvtSucc = true;
        break;
      }
    }
  ...
  if (GeneralizedExpInterleave && !ShouldBypass) {
    ...
    auto incrementTransPosition = [&CurrTransPosition, &MFMAChain, &PositionInChain, &CurrMFMAForTransPosition]() {
      ...
      //PositionInChain = CurrTransPosition / MFMAChains;
    };

    auto getNextTransPositionInChain = [&CurrTransPosition, &MFMAChain, &PositionInChain, &CurrMFMAForTransPosition]() {
      auto TempPosition = CurrTransPosition + 1;
    ...
    auto getNextTransMFMAChain = [&CurrTransPosition, &MFMAChain, &PositionInChain, &CurrMFMAForTransPosition]() {
      auto TempPosition = CurrTransPosition + 1;
      auto TempMFMAForTrans = CurrMFMAForTransPosition + MFMAEnablement;
      //errs() << "CurrTransPosition: " << CurrTransPosition << "\n";
      //errs() << "CurrMFMAForTransPosition: " << CurrMFMAForTransPosition << "\n";
    ...
    auto incrementMFMAPosition = [&CurrMFMAPosition, &MFMAChainForMFMA, &PositionInChainForMFMA]() {
      ++CurrMFMAPosition;
  ...
  SG = &SyncedSchedGroups[PipelineSyncID].emplace_back(
    ...
    SG->addRule(std::make_shared<LessThanNSuccs>(8, TII, SG->getSGID(), HasChainBetweenCvt));
    SG->initSchedGroup(SyncedInstrs[SG->getSyncID()]);
  ...
  SG = &SyncedSchedGroups[PipelineSyncID].emplace_back(
    ...
    if (!IsPostRA && MFMAChains) {
      ...
      SG->addRule(std::make_shared<EnablesNthMFMA>(MFMAEnablement + 1, TII, SG->getSGID(), true));
      SG->addRule(std::make_shared<IsPipeExp>(TII, SG->getSGID(), true));
      SG->addRule(std::make_shared<LessThanNSuccs>(8, TII, SG->getSGID(), HasChainBetweenCvt));
      SG->initSchedGroup(SyncedInstrs[SG->getSyncID()]);
    }
    ...
    auto MFMARatio = MFMAEnablement > ExpRequirement ? MFMAEnablement / ExpRequirement : 1;
...
if (UsesFMA) {
  SG = &SyncedSchedGroups[PipelineSyncID].emplace_back(
    ...
    if (!IsPostRA && MFMAChains) {
      ...
      SG->addRule(std::make_shared<EnablesNthMFMA>((((I*ExpRatio +J)/ExpRequirement) + 2) * MFMAEnablement + 1, TII, SG->getSGID(), true));
        SG->addRule(std::make_shared<IsPipeExp>(TII, SG->getSGID(), true));
        SG->addRule(std::make_shared<LessThanNSuccs>(8, TII, SG->getSGID(), HasChainBetweenCvt));
        SG->initSchedGroup(SyncedInstrs[SG->getSyncID()]);
...
void MFMASmallGemmSingleWaveOpt::applyIGLPStrategy(
  ...
  assert(
      (!IsPostRA || (DSWCount == 0 && DSWWithPermCount == 0 &&
...
/// \p IsPostRA specifes whether or not this is a reentry into the
/// IGroupLPDAGMutation. Since there may be multiple scheduling passes on the
/// same scheduling region (e.g. pre and post-RA scheduling / multiple
/// scheduling "phases"), we can reenter this mutation framework more than once
/// for a given region.
std::unique_ptr<ScheduleDAGMutation>
createIGroupLPDAGMutation(bool IsPostRA) {
