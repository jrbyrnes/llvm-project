...
private:
  ...
  public:
    ...
    LessThanNSuccs(unsigned Size, const SIInstrInfo *TII,
                               unsigned SGID, bool NeedsCache = false)
  ...
  public:
    bool apply(const SUnit *SU, const ArrayRef<SUnit *> Collection,
      ...
      return SU->getInstr()->getOpcode() == AMDGPU::V_CVT_I32_F32_e32 || SU->getInstr()->getOpcode() == AMDGPU::V_CVT_F16_F32_e32;

    }
    IsECvt(const SIInstrInfo *TII,
                               unsigned SGID, bool NeedsCache = false)
        : InstructionRule(TII, SGID, NeedsCache) {}
  };

  class IsNCvt final : public InstructionRule {
  private:
  ...
  public:
    bool apply(const SUnit *SU, const ArrayRef<SUnit *> Collection,
      ...
      auto MI = SU->getInstr();
      return MI->getOpcode() == AMDGPU::V_FMA_F32_e64 || MI->getOpcode() == AMDGPU::V_FMAC_F32_e32 || MI->getOpcode() == AMDGPU::V_MUL_F32_e32 || MI->getOpcode() == AMDGPU::V_PK_MUL_F32;

    }
    IsMul(const SIInstrInfo *TII,
                               unsigned SGID, bool NeedsCache = false)
        : InstructionRule(TII, SGID, NeedsCache) {}
  };
  ...
  public:
    ...
    IsNthDSR(unsigned Number, const SIInstrInfo *TII,
                               unsigned SGID, bool NeedsCache = false)
        : InstructionRule(TII, SGID, NeedsCache), Number(Number) {}
  };
  ...
  public:
    bool apply(const SUnit *SU, const ArrayRef<SUnit *> Collection,
      ...
      errs() << "IsNthMFMA with Number: " << Number << "\n";
      auto DAG = SyncPipe[0].DAG;
      auto TII = SyncPipe[0].TII;
      unsigned Counter = 0;
      if (Cache->empty()) {
        for (auto &ParseSU : DAG->SUnits) {
          if (TII->isMFMAorWMMA(*ParseSU.getInstr())) {
            if (Counter == Number) {
              errs() << "CacheMI: "; ParseSU.getInstr()->dump();
              Cache->push_back(&ParseSU);
              break;
            }
      ...
      errs() << "Cache MI NoNum: " << (*Cache)[0]->NodeNum << ", SUNodeNUm " << SU->NodeNum << "\n";
      return (*Cache)[0]->NodeNum <= SU->NodeNum;
    }
...
if (CvtCount == 32) {
  ...
  for (unsigned I = 0; I < (TransCount-9)/4; ++I) {
    ...
    SG = &SyncedSchedGroups[PipelineSyncID].emplace_back(
        SchedGroupMask::MFMA, 1, PipelineSyncID, DAG, TII);
    SG->addRule(std::make_shared<SecondMFMAChain>(TII, SG->getSGID(), true));
    SG->addRule(std::make_shared<IsNotSuccOfPrevGroup>(TII, SG->getSGID(), true)); 
    SG->initSchedGroup(SyncedInstrs[SG->getSyncID()]);
    SG = &SyncedSchedGroups[PipelineSyncID].emplace_back(
...
if (CvtCount != 32) {
    ...
    SG = &SyncedSchedGroups[PipelineSyncID].emplace_back(
        SchedGroupMask::VALU, 1, PipelineSyncID, DAG, TII);
    SG->addRule(std::make_shared<IsCvt>(TII, SG->getSGID(), true));
    if (true || !IsPostRA) SG->addRule(std::make_shared<IsSuccOfPrevNthGroup>(1, TII, SG->getSGID(), true));
    SG->initSchedGroup(SyncedInstrs[SG->getSyncID()]);
    SG = &SyncedSchedGroups[PipelineSyncID].emplace_back(
        SchedGroupMask::VALU, 1, PipelineSyncID, DAG, TII);
    SG->addRule(std::make_shared<IsFMA>(1,TII, SG->getSGID(), true));
...
for (int I = 0; I < (TransCount - 8)/4; I++) {
    unsigned Distance = I < 1 ? 1 : 0;
    unsigned Distance2 = I < 3 ? 1 + (2 - I) * 3 : 0;
    unsigned Distance3 = I < 2 ? (2 - I) * 2 : 0;
    ...
    if (true || !IsPostRA) SG->addRule(std::make_shared<Is2ndSuccOfPrevNthGroup>(I == 0 ? 11 + 1 *5 :15 + 1 *5, TII, SG->getSGID(), true));
    SG->initSchedGroup(SyncedInstrs[SG->getSyncID()]);
    SG = &SyncedSchedGroups[PipelineSyncID].emplace_back(
        SchedGroupMask::VALU, 1, PipelineSyncID, DAG, TII);
    SG->addRule(std::make_shared<IsCvt>(TII, SG->getSGID(), true));
    if (true || !IsPostRA) SG->addRule(std::make_shared<IsSuccOfPrevNthGroup>(I == 0 ? 5 : 6, TII, SG->getSGID(), true));
    SG->initSchedGroup(SyncedInstrs[SG->getSyncID()]);
    SG = &SyncedSchedGroups[PipelineSyncID].emplace_back(
        SchedGroupMask::VALU, 1, PipelineSyncID, DAG, TII);
    SG->addRule(std::make_shared<IsFMA>(1,TII, SG->getSGID(), true));
...
SG->addRule(std::make_shared<LessThanNSuccs>(10, TII, SG->getSGID(), true));
    ...
    if (!IsPostRA) SG->addRule(std::make_shared<SecondMFMAChain>(TII, SG->getSGID(), true));
    if (IsPostRA) SG->addRule(std::make_shared<IsNthMFMA>(MFMACount/2, TII, SG->getSGID(), true));
    if (true || !IsPostRA) SG->addRule(std::make_shared<Is2ndSuccOfPrevNthGroup>(I == 0 ? 12 + 1 *5 : 15 + 1 * 5, TII, SG->getSGID(), true));
    SG->initSchedGroup(SyncedInstrs[SG->getSyncID()]);
    SG = &SyncedSchedGroups[PipelineSyncID].emplace_back(
        SchedGroupMask::VALU, 1, PipelineSyncID, DAG, TII);
    SG->addRule(std::make_shared<IsCvt>(TII, SG->getSGID(), true));
...
SG->addRule(std::make_shared<LessThanNSuccs>(10, TII, SG->getSGID(), true));
    ...
    if (!IsPostRA) SG->addRule(std::make_shared<SecondMFMAChain>(TII, SG->getSGID(), true));
    if (IsPostRA) SG->addRule(std::make_shared<IsNthMFMA>(MFMACount/2, TII, SG->getSGID(), true));
    if (true || !IsPostRA) SG->addRule(std::make_shared<Is2ndSuccOfPrevNthGroup>(I == 0 ? 14 + 1 * 5 : 16 + 1 * 5, TII, SG->getSGID(), true));
    SG->initSchedGroup(SyncedInstrs[SG->getSyncID()]);
    SG = &SyncedSchedGroups[PipelineSyncID].emplace_back(
        SchedGroupMask::VALU, 1, PipelineSyncID, DAG, TII);
    SG->addRule(std::make_shared<IsCvt>(TII, SG->getSGID(), true));
...
SG->addRule(std::make_shared<LessThanNSuccs>(10, TII, SG->getSGID(), true));
    ...
    if (!IsPostRA) SG->addRule(std::make_shared<SecondMFMAChain>(TII, SG->getSGID(), true));
    if (IsPostRA) SG->addRule(std::make_shared<IsNthMFMA>(MFMACount/2, TII, SG->getSGID(), true));
    if (true || !IsPostRA) SG->addRule(std::make_shared<Is2ndSuccOfPrevNthGroup>(I == 0 ? 14 + 1 * 5 : 15 +  1 * 5, TII, SG->getSGID(), true));
    SG->initSchedGroup(SyncedInstrs[SG->getSyncID()]);
    SG = &SyncedSchedGroups[PipelineSyncID].emplace_back(
        SchedGroupMask::VALU, 1, PipelineSyncID, DAG, TII);
    SG->addRule(std::make_shared<IsCvt>(TII, SG->getSGID(), true));
...
SG->addRule(std::make_shared<LessThanNSuccs>(10, TII, SG->getSGID(), true));
    ...
    if (!IsPostRA) SG->addRule(std::make_shared<SecondMFMAChain>(TII, SG->getSGID(), true));
    if (IsPostRA) SG->addRule(std::make_shared<IsNthMFMA>(MFMACount/2, TII, SG->getSGID(), true));
    if (true || !IsPostRA) SG->addRule(std::make_shared<Is2ndSuccOfPrevNthGroup>(15 + 1 * 5, TII, SG->getSGID(), true));
    SG->initSchedGroup(SyncedInstrs[SG->getSyncID()]);
    SG = &SyncedSchedGroups[PipelineSyncID].emplace_back(
        SchedGroupMask::VALU, 1, PipelineSyncID, DAG, TII);
    SG->addRule(std::make_shared<IsCvt>(TII, SG->getSGID(), true));
...
SG->addRule(std::make_shared<LessThanNSuccs>(10, TII, SG->getSGID(), true));
    ...
    SG = &SyncedSchedGroups[PipelineSyncID].emplace_back(
        SchedGroupMask::VALU, 1, PipelineSyncID, DAG, TII);
    SG->addRule(std::make_shared<IsCvt>(TII, SG->getSGID(), true));
    SG->addRule(std::make_shared<IsSuccOfPrevNthGroup>(6, TII, SG->getSGID(), true));
    SG->initSchedGroup(SyncedInstrs[SG->getSyncID()]);
    ...
    if (!IsPostRA) SG->addRule(std::make_shared<SecondMFMAChain>(TII, SG->getSGID(), true));
    if (IsPostRA) SG->addRule(std::make_shared<IsNthMFMA>(MFMACount/2, TII, SG->getSGID(), true));
    if (true || !IsPostRA) SG->addRule(std::make_shared<Is2ndSuccOfPrevNthGroup>(14 + 1 * 4, TII, SG->getSGID(), true));
    SG->initSchedGroup(SyncedInstrs[SG->getSyncID()]);
    SG = &SyncedSchedGroups[PipelineSyncID].emplace_back(
        SchedGroupMask::VALU, 1, PipelineSyncID, DAG, TII);
    SG->addRule(std::make_shared<IsCvt>(TII, SG->getSGID(), true));
    ...
    errs() << "cvtgroup with ID: " << SG->getSGID() << "\n";
...
/*
  ...
  for (unsigned I = 0; I < CvtCount; ++I) {
    SG = &SyncedSchedGroups[PipelineSyncID].emplace_back(
        SchedGroupMask::VALU, 1, PipelineSyncID, DAG, TII);
    SG->addRule(std::make_shared<IsECvt>(TII, SG->getSGID(), true));
    SG->initSchedGroup(SyncedInstrs[SG->getSyncID()]);
    SG = &SyncedSchedGroups[PipelineSyncID].emplace_back(
        SchedGroupMask::VALU, 2, PipelineSyncID, DAG, TII);
    SG->addRule(std::make_shared<IsNCvt>(TII, SG->getSGID(), true));
    SG->initSchedGroup(SyncedInstrs[SG->getSyncID()]);
