...
static cl::opt<Optional<unsigned>> CutoffForExact(
    "amdgpu-igrouplp-exact-solver-cutoff", cl::init(None), cl::Hidden,
...
typedef SUnitsToCandidateSGsMap = DenseMap<SUnit *, SmallVector<int, 4>>;
...
typedef SUToCandSGsPair = std::pair<SUnit *, SmallVector<int, 4>>;
typedef SUsToCandSGsVec = SmallVector<SUToCandSGsPair, 4>;
...
bool PipelineSolver::solveExact() {
  ...
  assert(static_cast<size_t>(CurrConflInstNo) <
         PipelineInstrs[CurrSyncGroupIdx].size());
  SUToCandSGsPair CurrSGSU = PipelineInstrs[CurrSyncGroupIdx][CurrConflInstNo];
  LLVM_DEBUG(dbgs() << "Fitting SU(" << CurrSGSU.first->NodeNum
  ...
  auto I = CurrSGSU.second.rbegin();
  auto E = CurrSGSU.second.rend();
  assert(CurrSGSU.second.size() >= 1);
  for (; I != E; ++I) {
    int CandSGID = *I;
    int AddedCost = 0;
    std::vector<std::pair<SUnit *, SUnit *>> AddedEdges;
    ...
    LLVM_DEBUG(dbgs() << "Assigning to SchedGroup with Mask "
                      ...
                      << "\n");
    Match->add(*CurrSGSU.first);
    AddedCost = addEdges(SyncPipeline, CurrSGSU.first, CandSGID, AddedEdges);
  ...
  LLVM_DEBUG(dbgs() << "NOT Assigned (" << CurrSGSU.first->NodeNum << ")\n");
...
bool PipelineSolver::solveGreedy() {
  while (static_cast<size_t>(CurrSyncGroupIdx) < PipelineInstrs.size()) {
    SUToCandSGsPair CurrSGSU =
        PipelineInstrs[CurrSyncGroupIdx][CurrConflInstNo];
    int BestCost = -1;
    int TempCost;
    ...
    for (; I != E; ++I) {
      ...
      TempCost = addEdges(SyncPipeline, CurrSGSU.first, CandSGID, AddedEdges);
    ...
    if (BestGroupID != -1) {
      BestGroup->add(*CurrSGSU.first);
      std::vector<std::pair<SUnit *, SUnit *>> AddedEdges;
      addEdges(SyncPipeline, CurrSGSU.first, BestGroupID, AddedEdges);
...
void PipelineSolver::solve() {
  ...
  bool BelowCutoff = CutoffForExact && ProblemSize <= *CutoffForExact;
...
int SchedGroup::link(SUnit &SU, bool MakePred,
  ...
  for (auto A : Collection) {
    ...
    // If B is already recursive successor of A,
...
void IGroupLPDAGMutation::apply(ScheduleDAGInstrs *DAGInstrs) {
  ...
  for (int i = 0; i < DAG.SUnits.size(); i++) {
    PipelineOrderGroups.push_back({SchedGroupMask::VMEM, 1, 0});
    PipelineOrderGroups.push_back({SchedGroupMask::DS_READ, 1, 0});
    PipelineOrderGroups.push_back({SchedGroupMask::DS_WRITE, 1, 0});
...
std::unique_ptr<ScheduleDAGMutation> createSchedBarrierDAGMutation() {
  return EnableIGroupLP ? std::make_unique<SchedBarrierDAGMutation>() : nullptr;
}
