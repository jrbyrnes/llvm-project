...
class MFMAExpInterleaveSimpleOpt final : public IGLPStrategy {
private:
public:
  bool applyIGLPStrategy(
      DenseMap<int, SUnitsToCandidateSGsMap> &SyncedInstrs,
  ...
  MFMAExpInterleaveSimpleOpt(ScheduleDAGInstrs *DAG, const SIInstrInfo *TII)
      : IGLPStrategy(DAG, TII) {
    IsBottomUp = false;
  }
};

bool MFMAExpInterleaveSimpleOpt::applyIGLPStrategy(
    DenseMap<int, SUnitsToCandidateSGsMap> &SyncedInstrs,
    DenseMap<int, SmallVector<SchedGroup, 4>> &SyncedSchedGroups,
    AMDGPU::SchedulingPhase Phase) {
  // Count the number of MFMA instructions.
...
createIGLPStrategy(IGLPStrategyID ID, ScheduleDAGInstrs *DAG,
  ...
  case MFMAExpInterleaveSimpleID:
    return std::make_unique<MFMAExpInterleaveSimpleOpt>(DAG, TII);
  }
