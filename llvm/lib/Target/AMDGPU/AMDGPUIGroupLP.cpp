...
static cl::opt<unsigned long long> MaxBranchesExplored(
...
class PipelineSolver {
  ...
  void populateReadyList(SUToCandSGsPair &CurrSU, SmallVectorImpl<std::pair<int, int>> &ReadyList,
...
void PipelineSolver::reset() {
  ...
  for (auto &SyncPipeline : CurrPipeline) {
    for (auto &SG : SyncPipeline) {
      ...
      SG.Collection.clear();
      auto SchedBarr = std::find_if(TempCollection.begin(), TempCollection.end(), 
                        [](SUnit *SU){return SU->getInstr()->getOpcode() == AMDGPU::SCHED_GROUP_BARRIER;});
...
bool PipelineSolver::checkOptimal() {
  ...
  if (MaxBranchesExplored > 0 && BranchesExplored >= MaxBranchesExplored) DoneExploring = true;

  return (DoneExploring || BestCost == 0);
}

void PipelineSolver::populateReadyList(SUToCandSGsPair &CurrSU, SmallVectorImpl<std::pair<int, int>> &ReadyList,
                                       SmallVectorImpl<SchedGroup> &SyncPipeline) {
  ...
  for (; I != E; ++I) {
    ...
    else ReadyList.push_back(std::make_pair(*I, -1));
  }

  if (UseCostHeur) {
    std::sort(ReadyList.begin(), ReadyList.end(), [](std::pair<int, int> A, std::pair<int, int> B) {
...
bool PipelineSolver::solveExact() {
  ...
  for (; I != E; ++I) {
    // If we are trying SGs in least cost order, and the current SG is cost infeasible,
    // then all subsequent SGs will also be cost infeasible, so we can prune.
    ...
    if (CurrCost < BestCost || BestCost == -1) {
      if (solveExact()) {
        ...
        if (!FinishedExploring) return true;
      }
    }
...
void SchedGroup::link(SUnit &SU, bool MakePred) {
  for (auto A : Collection) {
    SUnit *B = &SU;
    if (A->getInstr()->getOpcode() == AMDGPU::SCHED_GROUP_BARRIER) continue;
    if (MakePred)
      std::swap(A, B);
