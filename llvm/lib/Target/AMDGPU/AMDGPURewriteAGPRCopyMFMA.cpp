...
bool AMDGPURewriteAGPRCopyMFMAImpl::run(MachineFunction &MF) const {
  ...
  for (auto &MBB : MF) {
    for (auto &MI : MBB) {
      ...
      const TargetRegisterClass *AssignedRC =
          TRI.getRegClassForReg(MRI, Def.getReg());
      ...
      for (const LiveInterval::SubRange &SR : LI.subranges()) {
        ...
        if (LIS.getInstructionIndex(MI) < SR.begin()->start) {
          ...
          bool Legit = TRI.getCoveringSubRegIndexes(
              TRI.getRegClassForReg(MRI, Src.getReg()), SR.LaneMask, Indexes);
