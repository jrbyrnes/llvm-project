...
static SDValue &convertToScalarType(SelectionDAG &DAG, SDLoc &SL, SDValue &Op, MVT ScalarType, bool IsSigned) {
  ...
  if (Size != ScalarType.getSizeInBits())
    Op = IsSigned ? DAG.getSExtOrTrunc(Op, SL, ScalarType) : DAG.getZExtOrTrunc(Op, SL, ScalarType);
...
static SDValue resolveSources(SelectionDAG &DAG, SDLoc SL,
  ...
  while (true) {
    ...
    auto FirstVal = convertToScalarType(DAG, SL, FirstElt->first, MVT::i32, IsSigned);
    auto SecondVal = convertToScalarType(DAG, SL, SecondElt->first, MVT::i32, IsSigned);

    Perms.push_back(DAG.getNode(AMDGPUISD::PERM, SL, MVT::i32, FirstVal, SecondVal,
    ...
    if (SecondElt == Srcs.end()) {
      auto EltVal = convertToScalarType(DAG, SL, FirstElt->first, MVT::i32, IsSigned);
...
SDValue SITargetLowering::performAddCombine(SDNode *N,
  ...
  if ((LHS.getOpcode() == AMDGPUISD::MUL_I24 ||
    ...
    if (ChainLength == 4 && Src0s.size() == 1 && Src1s.size() == 1 && Src0s.begin()->second == Src1s.begin()->second && Src0s.begin()->first.getValueSizeInBits() == 32 && Src1s.begin()->first.getValueSizeInBits() == 32) {
      ...
      if (UniqueEntries) {
        UseOriginalSrc = true;
        Src0 = convertToScalarType(DAG, SL, Src0s.begin()->first, MVT::i32, IsSigned);
        Src1 = convertToScalarType(DAG, SL, Src1s.begin()->first, MVT::i32, IsSigned);
      }
    }
    ...
    SDValue Src2 = convertToScalarType(DAG, SL, Src2s[ChainLength - 1], MVT::i32, IsSigned);

    std::optional<unsigned> Opcode;
    if (ST->getGeneration() == AMDGPUSubtarget::GFX11) {
      Opcode = IsSigned ? AMDGPU::V_DOT4_I32_IU8_gfx11
                                               : AMDGPU::V_DOT4_U32_U8_gfx11;
    }
    if (ST->getGeneration() == AMDGPUSubtarget::GFX10) {
      Opcode = IsSigned ? AMDGPU::V_DOT4_I32_I8_gfx10
                                               : AMDGPU::V_DOT4_U32_U8_gfx10;
    }

    if (ST->getGeneration() == AMDGPUSubtarget::GFX9) {
      Opcode = IsSigned ? AMDGPU::V_DOT4_I32_I8
