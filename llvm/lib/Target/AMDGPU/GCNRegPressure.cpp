...
bool GCNRegPressure::less(const MachineFunction &MF, const GCNRegPressure &O,
  ...
  const auto VGPROcc = std::min(
      MaxOccupancy, ST.getOccupancyWithNumVGPRs(getVGPRNum(ST.hasGFX90AInsts(), MaxArchVGPRs),
                                                DynamicVGPRBlockSize));
  const auto OtherSGPROcc = std::min(MaxOccupancy,
                                ST.getOccupancyWithNumSGPRs(O.getSGPRNum()));
  const auto OtherVGPROcc =
      std::min(MaxOccupancy,
               ST.getOccupancyWithNumVGPRs(O.getVGPRNum(ST.hasGFX90AInsts(), MaxArchVGPRs),
                                           DynamicVGPRBlockSize));
  ...
  unsigned OtherExcessVGPR =
      std::max(static_cast<int>(O.getVGPRNum(ST.hasGFX90AInsts(), MaxArchVGPRs) +
  ...
  unsigned ExcessArchVGPR = std::max(
      static_cast<int>(getVGPRNum(false, MaxArchVGPRs) + VGPRForSGPRSpills - MaxArchVGPRs),
      0);
  unsigned OtherExcessArchVGPR =
      std::max(static_cast<int>(O.getVGPRNum(false, MaxArchVGPRs) + OtherVGPRForSGPRSpills -
                                MaxArchVGPRs),
               0);
  // AGPR excess pressure conditions
  unsigned ExcessAGPR = std::max(
      static_cast<int>(ST.hasGFX90AInsts() ? (getAGPRNum(MaxArchVGPRs) - MaxArchVGPRs)
                                           : (getAGPRNum(MaxArchVGPRs) - MaxVGPRs)),
      0);
  unsigned OtherExcessAGPR = std::max(
      static_cast<int>(ST.hasGFX90AInsts() ? (O.getAGPRNum(MaxArchVGPRs) - MaxArchVGPRs)
  ...
  if (ExcessRP || OtherExcessRP) {
    ...
    if (SGPRDiff != 0) {
      unsigned PureExcessVGPR =
          std::max(static_cast<int>(getVGPRNum(ST.hasGFX90AInsts(), MaxArchVGPRs) - MaxVGPRs),
                   0) +
          std::max(static_cast<int>(getVGPRNum(false, MaxArchVGPRs) - MaxArchVGPRs), 0);
      unsigned OtherPureExcessVGPR =
          std::max(
              static_cast<int>(O.getVGPRNum(ST.hasGFX90AInsts(), MaxArchVGPRs) - MaxVGPRs),
              0) +
          std::max(static_cast<int>(O.getVGPRNum(false, MaxArchVGPRs) - MaxArchVGPRs), 0);
...
Printable llvm::print(const GCNRegPressure &RP, const GCNSubtarget *ST,
  ...
  return Printable([&RP, ST, DynamicVGPRBlockSize](raw_ostream &OS) {
    ...
    if (ST)
      OS << "(O"
         << ST->getOccupancyWithNumVGPRs(RP.getVGPRNum(ST->hasGFX90AInsts(), ST->getAddressableNumArchVGPRs()),
                                         DynamicVGPRBlockSize)
         << ')';
    OS << ", SGPRs: " << RP.getSGPRNum();
    if (ST)
      OS << "(O" << ST->getOccupancyWithNumSGPRs(RP.getSGPRNum()) << ')';
    OS << ", LVGPR WT: " << RP.getVGPRTuplesWeight(ST->getAddressableNumArchVGPRs())
...
bool GCNRPTarget::isSaveBeneficial(Register Reg,
  ...
  bool ShouldUseAGPR = SRI->isAGPRClass(RC) || (SRI->isVectorSuperClass(RC) && RP.getArchVGPRNum(AddressableNumArchVGPRs)  >= AddressableNumArchVGPRs);
  unsigned NumVGPRs =
      ShouldUseAGPR ? RP.getAGPRNum(AddressableNumArchVGPRs) : RP.getArchVGPRNum(AddressableNumArchVGPRs);
  ...
  auto printRP = [&MF](const GCNRegPressure &RP) {
    return Printable([&RP, &MF](raw_ostream &OS) {
      OS << format(PFX "  %-5d", RP.getSGPRNum())
         << format(" %-5d", RP.getVGPRNum(false, MF.getSubtarget<GCNSubtarget>().getAddressableNumArchVGPRs()));
    });
  };
