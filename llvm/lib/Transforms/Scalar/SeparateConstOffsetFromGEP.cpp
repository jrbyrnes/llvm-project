...
APInt ConstantOffsetExtractor::extractDisjointBitsFromXor(
  ...
  const KnownBits BaseKnownBits = computeKnownBits(BaseOperand, 0, SQ);
...
bool SeparateConstOffsetFromGEP::splitGEP(GetElementPtrInst *GEP) {
  ...
  bool OtherExtract = false;
  unsigned OtherOffset = 0;
  if (!ExtractBase) {
    auto GEPPtr = GEP->getPointerOperand();


    if (auto PO = dyn_cast<GEPOperator>(GEP->getPointerOperand())) {
      if (PO->hasAllConstantIndices()) {
        IRBuilder<> Builder(GEP);
        APInt Offset(DL->getIndexSizeInBits(cast<PointerType>(PO->getType())->getAddressSpace()), 0);

        Value *Temp = PO->stripAndAccumulateInBoundsConstantOffsets(*DL, Offset);
        ...
        if (PO->isInBounds())
          NewPtr = Builder.CreateConstInBoundsGEP1_32(
            PO->getSourceElementType(), Temp,
            Offset.getZExtValue(), "");
        else
          NewPtr = Builder.CreateConstGEP1_32(
            PO->getSourceElementType(), Temp,
            Offset.getZExtValue(), "");

        bool IsZero = Offset.getZExtValue() == 0;
        GEP->setOperand(GEP->getPointerOperandIndex(), IsZero ? Temp : NewPtr);
        RecursivelyDeleteTriviallyDeadInstructions(PO);
        if (IsZero) {
          RecursivelyDeleteTriviallyDeadInstructions(NewPtr);
        }
OtherExtract = !IsZero;
OtherOffset = Offset.getZExtValue();
Changed = true;

    NewBase = IsZero ? Temp : NewPtr;
  ...
  //errs() << "\nGEP with Ptr Op: "; GEP->getPointerOperand()->dump();
  //errs() << "Extract? " << ExtractBase << ", OtherExtract: " << OtherExtract << "\n";

  const int64_t BaseByteOffset = ExtractBase ? BaseOffset->getSExtValue() : OtherExtract ? OtherOffset : 0;
  ExtractBase |= OtherExtract;
  ...
  Changed |= canonicalizeArrayIndicesToIndexSize(GEP);
  ...
  if (!LowerGEP) {
    ...
    if (!TTI.isLegalAddressingMode(GEP->getResultElementType(),
      ...
      if (BaseByteOffset) {
        int64_t TempOffset = AccumulativeByteOffset - BaseByteOffset;
        bool IsLegal = TTI.isLegalAddressingMode(GEP->getResultElementType(), nullptr, TempOffset, true, 0, AddrSpace);
        if (IsLegal) {
          IsLayered = true;
          AccumulativeByteOffset = TempOffset;
          ExtractBase = false;
        }
      }
    ...
    if (!IsLayered) {
                                    ...
                                    errs() << "Not legal: \n"; GEP->dump();
                                    GEP->indices().begin()->get()->dump();
                                    errs() << "ByteOFfset: " << AccumulativeByteOffset << "\n";
      return Changed;
    }
    }
  }
...
bool SeparateConstOffsetFromGEP::run(Function &F) {
  ...
  for (BasicBlock *B : RPOT) {
    ...
    auto PO = dyn_cast<GEPOperator>(GEP->getPointerOperand());
    if (!PO)
      continue;

    if (GEP->getNumIndices() != 1 || PO->getNumIndices() != 1)
      continue;
    
    auto GEPOffset = GEP->indices().begin()->get();
    auto *OffsetValue = dyn_cast<ConstantData>(GEPOffset);

    if (!OffsetValue)
      continue;
    
    if (OffsetValue->getUniqueInteger().getZExtValue()!= 0)
      continue;
    
    GEP->setOperand(GEP->getPointerOperandIndex(), PO->getOperand(PO->getPointerOperandIndex()));
    GEP->setOperand(GEP->getPointerOperandIndex() + 1, PO->getOperand(PO->getPointerOperandIndex() + 1));
