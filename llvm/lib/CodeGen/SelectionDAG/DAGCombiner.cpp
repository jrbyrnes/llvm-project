...
calculateByteProvider(SDValue Op, unsigned Index, unsigned Depth,
  ...
  case ISD::EXTRACT_VECTOR_ELT: {
    ...
    // i8, this simply means the VID == StartingIndex. For non i8 cases, the
    // element will provide a range of bytes. For example, if we have a vector
    // of i16s, each element provides two bytes (e.g. V[1] provides byte 2 and
    // 3).
...
SDValue DAGCombiner::MatchLoadCombine(SDNode *N) {
  ...
  for (int i = ByteWidth - 1; i >= 0; --i) {
    auto P =
        calculateByteProvider(SDValue(N, 0), i, 0, /*VectorIndex*/ None, i);
    if (!P)
      return SDValue();
    ...
    // the index multiple by the byte size of each element in the vector.
